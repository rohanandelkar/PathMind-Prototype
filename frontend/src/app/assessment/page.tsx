  ...
  return (
    <div className="space-y-8 py-2 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Interactive Skill Assessments</h1>
          <p className="text-xs text-slate-400">Validate your skill progression. Quiz scores automatically adapt your roadmap timeline and skill gaps in PostgreSQL.</p>
        </div>
      </div>
      ...
      {lastResult && (
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">Score: {lastResult.score_percentage}% ({lastResult.correct_count}/{lastResult.total_questions})</span>
          </div>
          <h3 className="text-base font-bold text-white">{lastResult.topic} ({lastResult.difficulty})</h3>
          <p className="text-xs text-slate-300">{lastResult.adaptation_applied}</p>
        </div>
      )}
